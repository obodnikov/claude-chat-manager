...
class TestFileDiscoveryIntegration:
    ...
    def test_kiro_discovers_chat_files(self):
        """Test that Kiro source discovers .json session files (excluding sessions.json).
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create test files - Kiro uses .json for session files
            (test_dir / "session1.json").write_text(json.dumps({"chat": [{"role": "human", "content": "test"}]}))
            (test_dir / "session2.json").write_text(json.dumps({"chat": [{"role": "human", "content": "test"}]}))
            (test_dir / "chat1.jsonl").write_text('{"role": "human", "content": "test"}\n')
            (test_dir / "sessions.json").write_text(json.dumps([]))  # Metadata file, should be excluded
            
            # Test Kiro IDE source
            kiro_files = get_project_chat_files(test_dir, ChatSource.KIRO_IDE)
            
            assert len(kiro_files) == 2, f"Expected 2 .json session files, found {len(kiro_files)}: {[f.name for f in kiro_files]}"
            assert all(f.suffix == '.json' for f in kiro_files), "All files should be .json"
            assert set(f.name for f in kiro_files) == {'session1.json', 'session2.json'}
    ...
    def test_file_discovery_sorting(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Kiro uses .json for session files
            ...
            assert kiro_files[0].name == "new.json"
            assert kiro_files[1].name == "old.json"
